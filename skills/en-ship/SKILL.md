...
2a. **Start the run ledger.** `bash "$SKILL_DIR/scripts/ensemble-run-metrics" start --skill en-ship`. Nothing below is told where to record: the helpers find the run themselves. Fire-and-forget, and it never blocks a ship; `references/run-metrics.md` carries the event vocabulary.
...
15. **Close the run.** `bash "$SKILL_DIR/scripts/ensemble-run-metrics" finish`, then report as usual. **Every terminal path closes the run.** Success, a blocked preflight, a secret-scan stop, a failed push: each ends here.
