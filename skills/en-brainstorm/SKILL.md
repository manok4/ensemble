...
5. **Q&A loop.** Pull questions from `$ENSEMBLE_ROOT/references/socratic-questions.md`. **One question per turn.** Stop when scope, constraints, and the riskiest assumption are clear.
   - **Default to the host's blocking question tool** — `$QUESTION_TOOL` from `$ENSEMBLE_ROOT/references/host-detect.md` (`AskUserQuestion` on Claude Code, `request_user_input` on Codex), with its built-in free-text fallback — for narrowing / single-select questions; well-chosen options scaffold the answer without confining it. (Ensemble already routes through `$QUESTION_TOOL` in en-resolve-pr / en-simplify / en-debug.)
...
5a. **Product pressure test** (self-gating; adapted from `ce-brainstorm`). Before generating approaches, don't just interview, pressure-test whether the idea is real and well-framed. This is **internal analysis**: scan the opening and the dialogue so far for the product-rigor gaps below, and raise **only those that actually exist** as **open-ended probes folded into the conversation**, never a pre-flight checklist. A well-framed opening earns **zero** probes; one probe satisfies one gap. The gaps (full phrasing in `$ENSEMBLE_ROOT/references/socratic-questions.md` → "Product rigor gaps"):
   ...
   - **Budget rule.** Rigor probes (5a) and integration probes (5b) **count toward the depth question budget** (Lightweight 2–4, Standard 5–8, Deep 9–14 — see the depth table). They don't add a separate quota. On **Lightweight**, fire **at most one** rigor/integration probe — the single highest-signal gap — and skip the rest; a Lightweight brainstorm should not turn into a rigor interrogation. Standard/Deep have room for one probe per genuine gap within the budget.
