...
10. **Optional research.** Dispatch the `web-research` agent only when the user wants prior art or external best practice would materially change the recommendation. It never gets a turn of its own: on Standard/Deep it is one numbered item in the first frontier round, recommended answer **skip** unless a trigger in `references/research-dispatch.md` holds (a library the repo does not already use; the user on the fence between two approaches). Recognising a library's name is not knowing its current state; a name from a fast-moving area is a trigger, not a reason to skip. On Lightweight, skip unless asked.
