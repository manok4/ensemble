...
6. **Read context, bounded.** In one message, `AGENTS.md`, `CLAUDE.md`, project conventions, and the plans this plan names in `related:`; none depends on another's result. From `docs/foundation.md` read the frontmatter, then `grep -n '^#' docs/foundation.md` for the section index, then only the Functional Requirements entries for the R-IDs in the plan's `covers_requirements`. **Never read it whole**: it routinely runs past 2,000 lines, and the plan already carries the approach and file list the build needs.
...
9. **Phase loop (when phasing is on).** For each phase in `[P1, P2, P3, P4]`:
   ...
   - For each unit in the phase (dependency order):
     ...
     - **9e. Commit.** **Only after 9d exited 0**, with nothing edited since. If anything changed after that call, re-run it joined to the commit (`ensemble-unit-verify --unit U<N> --working && git commit …`) so a red assertion cannot reach a commit through inattention: three EN16 units were committed red because verifying and committing were two separate acts of attention. Conventional subject + U-ID + `phase: P<N>` trailer. **Stage only the unit's own files**, never `git add .`: a bare stage absorbs whatever was already in the index, which on a build that started from a dirty tree silently commits work the user never offered. If the unit needed a file that was already dirty, ask once whether to include or exclude it, and record the answer in the progress report.
   ...
   - **After-phase verification.** `bash "$SKILL_DIR/scripts/ensemble-unit-verify" --unit P<N> --range <phase-base>..HEAD --prefer-full-when-cheap`: lint, typecheck, and **the tests covering the files this phase touched** — not the full suite. On failure: stop; surface failing tests; offer investigate / commit-as-WIP-via-`--commit-wip` / abort. Do **not** advance to next phase.
     ...
     **The selection is resolved, never guessed (D105).** `ensemble-test-select` reports the tier that chose it: `graph` from the project's own `test_changed_command`, `impact-map` from its `test_impact:` prefixes, `sibling` from the filename heuristic, or `full-suite` when `test_full_seconds` says the whole suite is under five minutes. **Surface the tier**, because a selection nobody can audit is one nobody will notice is wrong. The cheap-suite tier exists for a failure a heuristic cannot fix: a test that anchors on file *content* is unreachable from the path that changed, and two of them stayed red for two hours on the EN16 build before the review found them.
