...
9. **Phase loop (when phasing is on).** For each phase in `[P1, P2, P3, P4]`:
   ...
   - For each unit in the phase (dependency order):
     ...
     - **9d. Verification gate.** Run unit tests + project lint, in one message since neither depends on the other. Failures → fix before committing (don't commit a broken unit).
