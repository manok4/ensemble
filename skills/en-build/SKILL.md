...
10. **Post-build phase (branch-level simplify → review → audit → learn).** Runs ONCE after all units commit. This is where ordinary units get their code-simplifier and Outside Voice review — at the branch level, not per-unit (D29).
    ...
    3. **Branch-level Outside Voice review (cross-agent — implementer ≠ reviewer).** The host just implemented every ordinary unit, so the branch-level review MUST run on the **peer** agent, not host-side personas. **Default: invoke `/en-review --peer-only --mode headless --base <merge-base>`** over the branch diff (the peer is the sole reviewer; Claude host → Codex; Codex host → Claude — D23). **Escalation:** classify the branch diff via `$ENSEMBLE_ROOT/references/diff-signal-detection.md`; when `is_high_stakes` is true (risk signals OR ≥150 exec lines OR unclassifiable — fail-closed), invoke **`/en-review --adversarial --mode headless --base <merge-base>`** instead, so the implementer's fresh-context host self-read AND the independent peer both run and reconcile (per `$ENSEMBLE_ROOT/references/adversarial-reconciliation.md`) — the agreement signal adds value even with a weaker host leg. en-review returns the findings envelope with a `reviewer` field (`cross-agent` for peer-only; `adversarial` for the escalated path; `single-agent-fallback` / `en-review-host-fallback` when no peer). **Host applies** eligible findings per `$ENSEMBLE_ROOT/references/severity.md` (auto-apply `safe_auto`; surface P0-disagreements / high-confidence security or architecture findings). Skip entirely with `--no-peer` (records the branch as review-skipped). The peer machinery lives in en-review (one implementation), not duplicated here.
