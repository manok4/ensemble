...
| `cap-exhausted-with-applied-findings` | Per-unit finalize loop hit the iteration cap with applied findings on the last pass. Already P1-warning-surfaced by `/en-build`'s per-unit finalize loop at cap exhaustion; the trailer makes it auditable. |
