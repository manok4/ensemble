...
2a. **Start the run ledger.** `bash "$SKILL_DIR/scripts/ensemble-run-metrics" start --skill en-foundation`. Fire-and-forget; the peer pass below records into it without being told where, per `references/run-metrics.md`.
3. **Detect mode.**
...
6. **Discovery loop.** Walk the topic groups in `references/foundation-questions.md`:
   ...
   **Frontier rounds, one per group.** Ask every open question in a group in **one numbered round**, each with your **recommended answer**, so a round is "confirm these, correct what I got wrong" rather than an interview; a question whose answer depends on another still open in the round waits for the next. Rigor probes stay one per turn. On **Lightweight**, ask one question per turn. Honor the question-count band per `references/foundation-questions.md`; a round's items count individually.
...
9. **Draft `docs/foundation.md`** using `references/templates/foundation-template.md`, from the ledger file. Settle structure and the hard calls in reasoning; write the document once, in the output. On a retrofit the file already exists: edit it surgically and rewrite only sections that mostly change.
...
11. **Outside Voice review** (`references/outside-voice.md` owns the contract and verdict handling). If `PEER_AVAILABLE=true`, ship the draft to the peer:
    ...
    - Apply, defer, or disagree per the routing table in `references/peer-brief.md`. Applied findings are surgical edits to the draft, never a rewrite of it.
...
17. **Hand off, then close the run.** `bash "$SKILL_DIR/scripts/ensemble-run-metrics" finish`. **Every terminal path closes the run**, a refused mode and an abandoned draft included. Suggest next step:
