...
| **Tautological** | The expected value is computed the way the code computes it, so the test passes by construction and can never disagree with the code. Expected values must come from an independent source: a known-good literal, a worked example, the spec. The tell: the expected value could be re-derived from the function under test. |
| **Horizontal slicing** | A test file complete before any of its subject exists. Bulk tests verify imagined behaviour and go insensitive to real change; a vertical slice is one test, one implementation, repeat. |
