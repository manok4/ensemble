...
11.4. **Validator sweep (opt-in / auto for P0-P1).** Per `$ENSEMBLE_ROOT/references/validator-dispatch.md`. Run a per-finding validator when `--validate` is set, or — by default — automatically over P0/P1 findings only (capped at 10). Each validator independently re-checks: real? introduced by this diff? not handled elsewhere? On `validated: false`, drop **P2/P3** findings (record the count + reasons in coverage); **keep P0/P1** unless the validator's reason is decisive (surface the disagreement). On validator infra failure, keep the finding (`validation: degraded`). `--no-validate` skips. Off the hot path — routine reviews with no P0/P1 pay nothing.
