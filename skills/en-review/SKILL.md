...
2b. **Read the effort/alias config overrides** (the two high-precedence layers only). This skill is the **SOLE resolver** (`peer-model-policy.md` (b)), but resolution is deliberately **split across two points** because the ladder's inputs do not exist yet at step 2:
   ...
   - **Overrides, read here:** the `--effort <low|medium|high|xhigh>` flag, then `$SKILL_DIR/scripts/ensemble-config-get peer_effort_override --legacy review_peer_effort_override --allowed low,medium,high,xhigh`. Either one is final; step 7b then skips the ladder.
   - **Models, read here:** `ensemble-config-get peer_model_alias --legacy review_peer_model_alias` (Claude alias), `peer_codex_model --legacy review_peer_codex_model` (Codex `-m`, else inherit), and `review_host_model_alias` (Agent-tool `model` for every persona under `--cross`/`--host`; else frontmatter). Host effort is the agent's `effort:` frontmatter, overridable per repo via `.claude/agents/dimension-reviewer.md` (`references/peer-model-policy.md` (c)). No `--model` flag, by design.
