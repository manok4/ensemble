...
7. **Conditional persona detection.** Per `$ENSEMBLE_ROOT/references/persona-dispatch.md`. In non-adversarial tiers this step only *selects the dimensions* for the peer brief (no host dispatch); in Adversarial it selects the host personas to dispatch in step 8.
   ...
   - Conditional (6) — fire when diff content matches: `security-reviewer`, `performance-reviewer`, `migrations-reviewer`, `api-contract-reviewer`, `reliability-reviewer`, `frontend-races-reviewer`. Each fires only on a match → zero cost on unrelated diffs.
   - Plus `learnings-research` to query `docs/learnings/` for relevant prior bugs/patterns/decisions.

   **7a. Lite brief (`--lite`).** Lite is a **peer-only tier** (step 6a) — it does NOT dispatch a host persona roster. `--lite` only narrows the **peer brief**: when `is_small_and_safe` is `true`, brief the peer (step 9) on **correctness + standards dimensions only** (skip testing / maintainability / learnings / conditionals). **Fail closed:** if `is_small_and_safe` is `false` for any reason (unknown line count, any uncounted non-code file, any risk signal, or any conditional persona matched), fall through to the **Standard** brief (full dimensions) regardless of `--lite` — the gate wins, the flag is advisory. (The only path that dispatches the host `correctness`+`standards`+`fast-pass` roster is the no-peer host fallback / `--host-only`; the optional `fast-pass` complement stays capped at anchor ≤ 50 and never counts toward corroboration promotion.)
...
11.4. **Validator sweep (opt-in / auto for P0-P1).** Per `$ENSEMBLE_ROOT/references/validator-dispatch.md`. Run a per-finding validator when `--validate` is set, or — by default — automatically over P0/P1 findings only (capped at 10). Each validator independently re-checks: real? introduced by this diff? not handled elsewhere? **One invariant:** on `validated: false`, drop **P2/P3** findings (record the count + reasons in coverage); **never auto-drop P0/P1** — keep them and mark `validation: disputed` so a human sees the finding and the validator's objection. On validator infra failure, keep the finding (`validation: degraded`). `--no-validate` skips. Off the hot path — routine reviews with no P0/P1 pay nothing.
...
| `--validate` / `--no-validate` | Per-finding false-positive validator (step 11.4, `$ENSEMBLE_ROOT/references/validator-dispatch.md`). `--validate` runs over all findings; default auto-runs for P0/P1 only (capped 10); `--no-validate` skips. Off the hot path. |
