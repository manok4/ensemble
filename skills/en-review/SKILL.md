...
2b. **Read the effort/alias config overrides** (the two high-precedence layers only). This skill is the **SOLE resolver** (`peer-model-policy.md` (b)), but resolution is deliberately **split across two points** because the ladder's inputs do not exist yet at step 2:
   ...
   - **Models, read here:** `ensemble-config-get peer_model_alias --legacy review_peer_model_alias` (Claude alias), `peer_codex_model --legacy review_peer_codex_model` (Codex `-m`), and `review_host_model_alias` (Agent-tool `model` for every persona under `--cross`/`--host`; else frontmatter). With it unset, `$SKILL_DIR/scripts/ensemble-agent-model` decides for `dimension-reviewer` per `references/agent-dispatch.md` (D103). Host effort: the agent's `effort:` frontmatter; a repo copy at `.claude/agents/dimension-reviewer.md` overrides (policy (c)). No `--model` flag.
...
8. **Dispatch, per review mode.** `--peer` (default) dispatches the peer alone: step 7's detection fed the effort tier and nothing else, and no persona roster runs. `--host` dispatches the persona roster alone. Only `--cross` dispatches both, and only `--cross` reaches the reconciliation in step 10.
    ...
    **`--cross`: the peer first, then the personas in ONE batch.** Start the peer detached (step 9), then one message with multiple `Agent` tool calls for the roster, each carrying `model:` from step 2b (`review_host_model_alias`, else the resolver's `$AGENT_MODEL`; none if both empty). The peer is **blind** to persona findings (step 9), so nothing orders it after the roster. Collect the personas, then wait for the peer.
9. **Outside Voice peer (runs in `--peer` and `--cross`; `--peer` is the default and makes it the sole reviewer).** Dispatch a cross-agent peer pass over the diff (peer is the other agent per D23):
   ...
   - Translate step 2b's result: `eval "$($SKILL_DIR/scripts/ensemble-peer-flags --effort <tier> --peer-cmd "$PEER_CMD" --model-alias <alias> --codex-model <codex>)"` → `$PEER_MODEL`, `$PEER_EFFORT`.
