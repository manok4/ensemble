...
5. **Apply fixes.** Aggregate findings; apply each directly, as a surgical edit at the finding's location. Never rewrite a file to apply a finding: a simplify pass that reflows a whole file produces a diff the branch review reads as reformatting, and the behaviour check in step 6 cannot tell a reformat from a change. Skip false positives silently (don't argue or ask).
...
6. **Verify behavior preserved.**
   - Run typecheck + lint over the project, in the same message as the scoped tests below since none depends on another (fast; catches broken imports, dropped narrowings, dead code other modules reference).
