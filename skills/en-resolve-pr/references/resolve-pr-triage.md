...
| State | Action |
|---|---|
| Only the original reviewer comment(s); PR author hasn't responded | **New** — process |
| PR author has replied substantively, acknowledging or deferring (e.g. *"need to align on this"*, *"thinking through this before making a call"*, presents options without resolving) | **Pending decision** — skip; surface in the summary as "still pending from a previous run" |
...
**Silent drop rule.** Non-actionable items are dropped without narration. **Do not** announce, list, or count dropped items in the conversation, the task list, or the summary.
...
Replies are matched by the `> [quoted excerpt]` pattern the SKILL's reply step mandates: a PR-author comment whose body starts with a markdown blockquote line counts as a reply if the quoted text overlaps with the original feedback's body. The triage step does this matching at the SKILL layer, not in the script — the script's job is to expose the raw `pr_author_replies` so the SKILL has the data.

If the user replied manually in a different format (no `> ` quote prefix) and `/en-resolve-pr` doesn't recognize it, that item is re-processed; declining it again is the recovery.

## Outcome

After triage, each item is exactly one of:

- **new** — process (rubric → fix → validate and push → reply and resolve)
- **pending** — skip; report in summary
- **silent-drop** — discarded; never mentioned

If no items end up in the `new` bucket, the SKILL skips validation, commit and push and goes straight to the merge-readiness check and the summary.
