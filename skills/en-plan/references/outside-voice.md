...
2. `ensemble_peer_invoke` (sourced from `$SKILL_DIR/scripts/ensemble-peer-invoke`) runs the peer with `ENSEMBLE_PEER_REVIEW=true` set, reading the prompt file on stdin. It owns the `timeout` wrapper (`peer_timeout_seconds` overrides; default 600, or 1200 for `--access read-tree`; `timeout` or `gtimeout` must be on PATH), failure classification (`auth` / `unknown` / `timeout` / flag drift), the single bounded retry, unwrapping the CLI's transport envelope and recovering fenced or prose-wrapped JSON, the served-model receipt (`model_actual`), and the `peer_decision` object it prints (`references/peer-contract.md`). `--schema <file>` binds the CLI's output schema (`--json-schema` on Claude, `--output-schema` on Codex). For a peer that may run for minutes, the same helper offers `ensemble_peer_start --job-dir <dir> …` (forks and returns at once where the fork survives; runs the peer in place where it does not, same job dir either way), `ensemble_peer_wait <dir> --max-secs <n>`, `ensemble_peer_result <dir>` and `ensemble_peer_reap <dir>`, so no host tool call has to stay open for the peer's lifetime and host personas can run alongside it (D81).

**The isolation flags are Claude-CLI-only, keyed on the command and applied by the helper in two access modes.** Every Claude peer gets `--strict-mcp-config --mcp-config '{"mcpServers":{}}' --disable-slash-commands --no-session-persistence --setting-sources project`: no MCP servers, no skills, no session state, no user-level settings. `--access none` (the default) adds `--tools ''`, so the caller's `--max-turns 1` is reliably one model response; a document review, or a lite code review, needs nothing beyond the artifact in the prompt. `--access read-tree` adds `--tools Read,Grep,Glob --permission-mode dontAsk` and a turn cap of 150 (`ENSEMBLE_PEER_MAX_TURNS`), so a code reviewer can open the caller of a changed function and the test beside it; a turn is one model-to-tool round trip inside the peer, and the host still receives one report. No Bash, Edit, Write, Agent, web or MCP in either mode, which is D30 held mechanically. `codex exec` gets `-s read-only` in both modes and none of the Claude flags. A CLI that rejects one of these flags costs one retry without the set, reported as `dropped-isolation-fragment`; `ENSEMBLE_PEER_ISOLATION=off` disables the set for a session.
...
- `$PEER_TURNS` is `--max-turns 1` for a `claude -p` peer and empty for `codex exec`, which is single-shot. Under `--access read-tree` the helper replaces it with its own cap.
