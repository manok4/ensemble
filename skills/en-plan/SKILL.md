...
13. **Outside Voice review with finalize loop.** If `PEER_AVAILABLE=true` (and `--no-peer` not set):
    - Build the prompt per `references/outside-voice.md` — the plan-specific block in the template will instruct the peer to cross-check risk classification, gated-flag presence, and dependency-vs-phase invariants.
