...
│       verdict was `revise` AND ≥1 finding applied AND              │
│       `re_review_count < --max-per-unit-iterations` (default 1):  │
│       re-review the post-fix diff with previous-iteration         │
│       context, increment counter. Loop back to step 6. On          │
│       `approve` or cap, continue.                                  │
│   10. Commit (conventional message + U-ID + `phase: P<N>` +       │
...
**Counter semantics.** `re_review_count` starts at **0** and increments after each re-review pass. The initial inline review at step 6 does NOT count toward it. Default cap=1 → exactly one re-review whenever step 6 produced `revise` with applied findings. (Counting from 1 would silently break the loop with the same default — see `build-handoff.md` for the full reasoning.)

Same routing as `build-handoff.md` for verdict / condition / action — see the table there. Set `--max-per-unit-iterations 0` to disable; raise the cap when units routinely hit the cap-hit P1 warning.
...
- **No turn cap for the Codex worker (EN10):** `codex exec` has no turn-cap flag and is single-shot — it runs the unit's work to completion and exits — so there is nothing to cap. (An earlier draft added a Codex-worker turn cap; that flag does not exist on `codex exec` and would error, so it is removed.)
...
2. **`/en-review --peer --mode headless`** over the branch diff — the host implemented every ordinary unit, so a **cross-agent peer is mandatory** (Claude host → Codex; Codex host → Claude; D23) and carries the implementer ≠ reviewer property. The host personas run **alongside** it (D46, superseding the former `--peer-only`): they are fresh-context sub-agents that never saw the implementing reasoning, so they do not weaken the cross-agent property, while `--peer-only` was discarding every host-only standards / testing / maintainability finding. en-review owns the dispatch (`bin/ensemble-build-peer-prompt --artifact-type code` → `$PEER_CMD`, `ENSEMBLE_PEER_REVIEW=true`), reconciles the two sources into buckets, and returns the findings envelope; the host applies eligible findings (D30) and never auto-applies `conflicting` ones. Fallbacks (handled inside en-review): single CLI → fresh-subprocess (`single-agent-fallback`); no peer at all → host persona roster (`reviewer: en-review-host-fallback`).
