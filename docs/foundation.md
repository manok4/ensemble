...
- **D36. Never issued.** No entry was ever written under this number: it is absent from every version of this file in git history, and nothing in `docs/`, `skills/` or `tests/` cites it. A number was burned during drafting and the gap went unnoticed until 2026-09-04, because the log's lint checked only that numbers ascend without repeating and a gap passes both. **It is recorded rather than filled.** Renumbering D37 and everything above it would invalidate every citation in the repo, including the amendment chains (D46 amends D35, D52 supersedes it) that are the log's whole point. A declared gap costs one line; a renumber costs the ability to cite anything. `tests/lint/decision-log-order.test.sh` now requires the sequence to be contiguous **except** for numbers declared this way, so this one is allowed by name and a new accidental gap fails.
...
- **D67. `/en-loop` filed `--max-iterations` as a gnhf pass-through, which would have deleted its review layer.** en-loop drives its checkpoint cadence by running gnhf in bounded chunks: **gnhf's own `--max-iterations` carries `--review-every`**, so each chunk stops at a boundary and the peer review can run. The Flag-ownership paragraph listed `--max-iterations` among the flags forwarded to gnhf. An agent following it forwards the user's overall cap, and `--max-iterations 40 --review-every 5` becomes one forty-iteration chunk with **no checkpoint review at all**, losing half of what en-loop adds over a bare gnhf run. Nothing in the run would say so. **The guard for that paragraph checked that it existed**, greping for the words "Flag ownership", "advertises", "en-loop-owned" and `timeout || gtimeout`, and never which flag sat on which side, so it passed on the contradiction it was written to prevent. Ownership is now declared per row in the flags table and asserted per row, because a row is structure and a sentence is prose: the first rewrite parsed the ownership sentence and, when the defect was reintroduced, went red through a fallback in its own string surgery rather than the check meant to catch it. `--agent` was in neither list and is now named a pass-through. **What this costs:** every new flag has to declare a side before the suite goes green. **A correction:** the audit predicted a stale `--peer-only` here, since D46 says `/en-loop` keeps that flag. en-loop already says `--peer`, which is correct after the 2026-09-01 rename. The stale spelling is in `en-review`'s own steps and in D46's sentence, not in en-loop, and is left for a separate change. Enforcement tier: `tests/lint/en-loop-skill.test.sh`, 31 assertions.
- **D68. Eight `bin/ensemble-*` paths were cited 85 times and resolved nowhere.** Not at this repo's root, not installed project-local by `/en-setup`, not carried under any skill's `bin/`. Every one existed as `scripts/<name>` in the citing skill, so this was a stale prefix and not missing payload, **which is exactly why it survived**: the file was always one directory over, and an agent that guessed correctly saw nothing wrong. The worst was `bin/ensemble-detect-host`, cited 33 times including "skills invoke `bin/ensemble-detect-host` as before, and the script handles caching transparently". A skill that gave up on that path and sourced the inline snippet in `host-detect.md` instead **lost the recursion-guard short-circuit and the session cache**, neither of which is in the snippet: it still worked, only slower, which is the kind of defect nobody reports. `bin/` is now correct for exactly the four scripts `/en-setup` copies into the consuming project, and the guard derives that list from en-setup rather than hardcoding it. **The rule needed a second half:** `/en-loop`, `/en-learn` and `/en-resolve-pr` all carried executables with no `$SKILL_DIR` anchor block, so they could not express the correct path at all, and en-learn and en-resolve-pr invoked theirs as a bare `scripts/<name>` that resolves against the user's project and exits 127. **What this costs:** a contract may no longer name the helper behind a value, so `en-plan`'s `peer_review_plan_hash` row now promises the value is canonical rather than naming the script that mints it, which is the right shape for a contract and loses a useful pointer. **Two bugs in the guard itself, both found by its own negative controls:** a newline-separated exemption list that a space-delimited `case` could never match, so every exemption failed open; and `ls a/* b/*` exiting non-zero when only one glob matches, which skipped the anchor check for every skill but one. Enforcement tier: `tests/lint/bin-path-citations.test.sh`, 4 assertions.
- **D69. The skill catalogue said fourteen when there were sixteen, and a lint held it there.** `en-flow` and `en-simplify` both had §5.1 rows and neither was in the §5 sentence. `tests/lint/en-loop-foundation.test.sh` asserted the literal word **"Fourteen"**, with a comment saying the count is asserted "because it is the cheapest way to notice a skill added or removed". It did not notice, twice, and it would have failed anyone correcting the number: **a count pinned to a word only tests that nobody fixed it.** The check now counts `skills/` and compares, and separately asserts that every directory appears in the sentence, which is what the literal check could never do. The 5.1 table also numbered its rows 1 to 17 across 16 rows, skipping 11 where a retired skill had been; the numbering is now asserted contiguous. **What this costs:** adding a skill without touching the catalogue is now a test failure rather than a silent drift, which is the intent, and the number-word table in the test needs extending past twenty.
- **D70. `/en-flow` described a review that had been replaced, and captured over a user who had said no.** Two defects in the smallest skill in the suite, both from restating a callee's internals. **The review:** stage 2 said en-build ran a peer-only post-build review and cited **D35**. en-build has invoked `/en-review --cross` since **D46**, whose whole subject was that peer-only *"discarded every host-only finding"*, the standards, testing and maintainability categories, and D52 then amended D46. en-flow cited the one decision that was overruled twice on the exact sentence it supported, so a reader believed the build gate produced none of those findings. **The capture:** en-build's learning checkpoint records four canonical values, and answering `skip` records `intentionally_skipped`. en-flow's learn step said it *"only acts if the build's hand-off was skipped/**declined**"*, so a person asked "Worth filing learnings from this build?" who answered `skip` got a capture from the next step anyway. en-build spends four distinct values distinguishing *nobody was asked* from *someone said no*; collapsing them threw that away. The step now branches on all four and acts only on `ci_environment` or a missing value. **Also:** the Process ran 2, 3, four unnumbered stage headings, 5, with no step 1 and no step 4, carrying two numbering schemes at once; `$ARGUMENTS` appeared once in the repo, a Claude Code slash-command substitution used in a skill body; and `<promise>DONE</promise>` sat in the stage graph, inherited from the compound-engineering skill this pipeline was adapted from, emitted and read by nothing. **What this costs:** en-flow now restates less of en-build and points at it more, which is correct but means a reader needs both files open. **The guard that matters** is not any of these fixes: `/en-flow` is the only skill that branches on four other skills' frontmatter, and nothing checked that the values it quotes still exist on the other side. A rename in `en-plan` would leave en-flow gating on a value that never appears, which stalls the pipeline or gets reasoned past, and says nothing either way. Enforcement tier: `tests/lint/en-flow-gate-contracts.test.sh`, 16 assertions, which reads both en-flow files because the reference restates every gate and a fix applied to one is not a fix.
...
- **D95. The peer effort enum gains `xhigh` as an opt-in; the ladder rungs are re-measured, never renamed (amends D45).** The peer helpers accepted `low|medium|high` and rejected anything else with exit 2. The current model generation exposes five tiers, and its prompting guide says two things that bear on Ensemble: effort is the primary cost-versus-quality control, and a tier name does not mean the same amount of reasoning across model generations, so a ladder tuned on one model is not a ladder on the next. `ensemble-peer-flags`, `peer-model-policy.md` and `/en-review`'s flag and config surfaces now accept `xhigh`; it reaches the peer only through the `--effort` flag or the config override, never from the ladder, whose rungs still resolve `low`, `medium` or `high`. A peer CLI that rejects the value drops the fragment as `dropped-effort-fragment` under the existing fail-soft, so a stale CLI degrades the call rather than the review. `max` is not offered: on a long artifact it drafts the reply in reasoning and again in output. **Deferred, deliberately:** moving the security or destructive rung to `xhigh`, and re-binding the bundled agents' `model:` tiers, both wait on a measured comparison over real diffs; the guide's own advice is to re-run the sweep, not to assume.
- **D96. `/en-setup` asks its opt-ins in one round and then runs to the end (amends D63, D92).** The State 2 retrofit stopped up to seven times in one run: plan archival, sweep cadence, local config, guardrail scope, gnhf, the review action and `REVIEW.md` each raised their own y/n at their own step, with the lint PR check an eighth in the verification phase. `/en-brainstorm` and `/en-plan` moved to frontier rounds with recommended answers in D47 and D48; setup, the most mechanical flow in Ensemble, was still an interview. The current model generation's guidance sharpens the cost: a model that asks permission readily for work already requested, run under a skill that mandates seven prompts, produces seven pauses in a flow the user started precisely so they would not have to drive it. Step 1a now issues every probe in one message and puts every item the probes left open to the user as one numbered round with a recommended answer per item; the steps that used to prompt read the round's answers, and from there the install runs through the final verification without stopping, printing one line per step. Recorded declines stay unlisted, as before. **Measured:** seven to eight blocking prompts per retrofit → one. SKILL.md 31.1KB → 31.8KB, under its pin, the round's table paid for by shorter prompt prose. **What this costs:** the round is asked before any artifact exists, so a user who wanted to see the skeleton land before deciding on the workflow now decides first; the report at the end still lists every choice.
- **D97. `/en-foundation` interviews in frontier rounds, keeps its ledger on disk, and does not wait for the retrofit scan (amends D47, D48 by extension).** The discovery loop was the last one-question-per-turn interview in Ensemble: up to twelve groups, a Deep band of 30–45 questions, each its own round trip, after D47 and D48 had moved brainstorm and plan to frontier rounds with recommended answers. Each group is now one numbered round; a question that depends on another still open waits for the next round; rigor probes and Lightweight keep one-per-turn, as the other two skills do. **Two more from the same review.** The known-facts ledger D-dated 2026-09-01 lived only in the dialogue, and a discovery loop is long enough for the context to be compacted under it, after which the draft at step 9 is written from a summary of the interview rather than the interview; the ledger is now a file under `/tmp/ensemble/en-foundation/<run-id>/` and the draft reads it. On a retrofit, `repo-research` was dispatched and waited on before any question was asked, though §1, §2, §3 and §14 cannot be answered from code; it is dispatched first and those groups are asked while it runs. Step 9 and the peer-finding application say the draft is edited surgically on a retrofit, where the file already exists and here runs to 300KB. **What this costs:** a round of six questions with recommendations is denser than six turns, which is the trade D47 already accepted; and a ledger on disk is one more file a run leaves behind under `/tmp/ensemble/`, alongside the review envelopes.
- **D98. A run that can outlast its context keeps its ledger on disk, and the report is derived from the file.** `/en-build`'s per-unit outcomes, `/en-qa`'s selected flows and their results, `/en-flow`'s stage outcomes and `/en-resolve-pr`'s item list all lived only in the conversation. Each of those runs is long enough for a client-side compaction to land in the middle of it, and a skill cannot set the summariser's instructions, so what survives a compaction is whatever the summary kept. For `/en-qa` that is exactly the failure D85's outcome bar exists to prevent: a flow dropped from memory reads as a flow that passed. `/en-review` already writes its envelope to `/tmp/ensemble/en-review/<run-id>/` and `/en-loop` reconstructs from git by design; the four skills now do the same. `/en-qa` writes `.test-output/qa/<run-id>/flows.json` before the first flow and updates it per flow; `/en-build` appends each unit's record to `/tmp/ensemble/en-build/<run-id>/ledger.json` and derives the final summary and the audit table from it; `/en-flow` records the plan path and en-build's `learning_checkpoint:` in `state.json` as each stage returns; `/en-resolve-pr` writes its numbered items to `items.json` and updates verdicts as they land; `/en-foundation`'s ledger moved to disk under D97. **Alongside, from the same prompting-guide review**, the three skills that edit code carry the scope rule the guide recommends: a pre-existing bug or behaviour the task does not mention is a follow-up, not a fix, unless the requested behaviour cannot work without it, and committed tests are sized like the neighbouring files; `/en-build` and `/en-resolve-pr` edit surgically rather than rewriting files; `/en-build`, `/en-qa` and `/en-flow` print one line per unit, flow or stage as it completes, since the current model generation narrates less during long tool chains. **What this costs:** each run leaves one more file under `/tmp/ensemble/` or `.test-output/`, and a skill that derives its report from a file has to keep writing the file, which the guard in `tests/lint/run-ledgers.test.sh` checks only by presence of the path.
- **D99. The prompting-guide one-liners: autonomous framing in the two fresh-model prompts, and batching, search, edit and long-output lines where each applies.** A read of the sixteen skills against the current model generation's prompting guide (`docs/reviews/fable-5-1-prompting-guide-skill-audit.md`) found most of them already ahead of it; what remained were sentences, placed where a behaviour the guide names could cost a run. **Two fresh-model prompts.** `/en-loop`'s worker prompt and `/en-sweep`'s CI invocation are the only places Ensemble composes a whole prompt for a model that starts with no context; neither said that nobody is watching. A worker that ends an iteration on "next I would" produces a rollback, and a CI runner that describes the PR it would open passes the `num_turns` guard while doing nothing, since that guard catches a runner that did not execute, not one that executed and stopped short. Both prompts now open with the autonomous framing and the check-your-last-paragraph rule; the sweep wrapper also supplies its prompt itself, because the workflow calls it bare and `claude -p` with no prompt reads an empty stdin. **Batching.** A resolve-context step that lists independent reads now says to issue them in one message: `/en-build` step 6 and 9d, `/en-plan` step 4, `/en-ship` step 1, `/en-debug`'s code-mode investigation, `/en-simplify` step 6, `/en-guardrail`'s status reads, `/en-setup`'s probes under D96; `/en-sweep` dispatches its architecture scan first and lints while it runs, and `/en-plan` asks its round-1 question while research runs instead of awaiting it. **Search.** The `web-research` trigger in `/en-plan` and `/en-brainstorm` gated on a judgement the model makes from memory; it now says that recognising a name from a fast-moving area is a reason to search, not to skip. **Edits and long outputs.** The skills that edit a long existing document say to edit it surgically (`/en-plan`'s finalize loop, `/en-learn`'s foundation and pointer-map syncs, `/en-sweep`'s batches, `/en-simplify`'s apply step); the three that write documents of several hundred lines say to settle structure in reasoning and write once (`/en-plan`, `/en-foundation`, `/en-brainstorm`). **Deliberately not done:** no anti-formatting rules were added or removed, since every output template already asks for the structure it wants; the mannered-prose pass over the skill files themselves is a style change with no expected behaviour change and was left out. **What this costs:** a few hundred bytes per skill inside the size pins, and one more behavioural test with a stubbed `claude` on PATH for the sweep wrapper.
- **D100. Operator-set defaults for the reviewers' model and effort, and for the sweep runner (amends D44, D45, D86's tier binding rule).** Ensemble never names a concrete model, and that rule stands for its own files; what was missing was a place for the operator to name one. `/en-review` already read `review_peer_model_alias` and `review_peer_effort_override` for the Claude peer. It now also reads `review_peer_codex_model`, passed as `-m` when the peer is Codex (the ID lives in the operator's config, not in Ensemble), and `review_host_model_alias`, passed as the Agent tool's `model` parameter to every host persona under `--cross` and `--host`; the envelope and summary report `host_model` next to the peer's alias. **The asymmetry, stated rather than hidden:** the Agent tool takes a per-call `model` and nothing for effort, so host effort is the `dimension-reviewer` frontmatter (`effort: high`, the shipped default) and a repo that wants another places a copy at `.claude/agents/dimension-reviewer.md`, which outranks the plugin's. No `review_host_effort` key exists because no dispatch could honour it. `agent-dispatch.md`'s rule that call sites omit model overrides gains its one exception: an operator-configured alias is the operator deciding, not the caller. **The sweep runner** reads `ENSEMBLE_SWEEP_MODEL` and `ENSEMBLE_SWEEP_EFFORT` from the workflow's `env:`, sourced from repository Actions variables, and maps them to `--model` / `--effort` on Claude or `-m` / `model_reasoning_effort` on Codex; malformed values are dropped, never passed. Actions variables rather than config keys because the repo-local YAML is gitignored and never reaches CI, which the config template now says. A manual `/en-sweep` runs on the session's model and is unaffected. **What this does not decide:** the values. Defaults are unchanged (`sonnet` peer alias, ladder effort, `opus` host at `high`, runner CLI default); the knobs let a repo run both reviewers on a chosen model and tier today, and the shipped defaults still wait on the D95 eval. **Cost:** `/en-setup` does not write the project-level agent override; it is documented in the config template and the policy, and a setup step for it would need en-setup's size pin to give way.
- **D101. `/en-sweep` runs on a dedicated machine through Codex, on a launchd schedule, and its runner merges the PRs (amends D27, D65, D94, D100).** The sweep ran as a GitHub Actions cron with a wrapper that resolved whichever CLI the runner had, a `GITHUB_TOKEN` scoped to the job, repository variables for the model, and `gh pr merge --auto` that left the merge to branch protection. Mano wanted it off GitHub, on a machine he controls, through Codex specifically, with every run ending in merged PRs and no one in the loop, and still with a model he sets. **What replaced it.** `scripts/ensemble-sweep-runner` is the scheduled entry point: it reads a repo list from `~/.ensemble/sweep-repos`, and per checkout refuses a dirty tree, fast-forwards the default branch, runs the activity gate, launches `codex exec --json -s workspace-write` with network on and approvals off, and then merges. `scripts/install-sweep-schedule` writes the launchd agent (`daily` / `weekly` / `monthly`, local time), pins a PATH that has `codex`, `gh` and `jq`, and carries the machine-wide `ENSEMBLE_SWEEP_MODEL` / `ENSEMBLE_SWEEP_EFFORT`; it is run by a person on that machine, for the reason the guardrail's global install is print-only. **The model** is `sweep.model` / `sweep.effort` in the repo's config on the sweep machine, then the plist's defaults, then Codex's own; D100's Actions variables are gone with the workflow. **Merging moved out of the skill and into the runner**, deterministically: the skill records `merge_eligible` per PR in `.ensemble/sweep-result.json` (report-only review found no P0/P1, `auto_merge_enabled` not false) and never merges; the runner waits for each eligible PR's checks to finish green and runs `gh pr merge --squash --delete-branch`, leaving a PR open, with the reason logged, on a failed check, a timeout, or a `BLOCKED` state, which is branch protection wanting a review the machine's identity cannot give. That last case is the one thing "without manual intervention" cannot override from Ensemble's side: it is a repository setting, and the security model says so. **The result file is also the inert-run guard.** The old wrapper failed a run whose envelope showed zero turns, which could not tell "ran and did nothing" from "described what it would do"; no valid result file after a run now means the skill did not execute, nothing merges, and the repo is logged as failed. **Retired:** the workflow template, the CI wrapper, the three project-local `bin/` copies `/en-setup` installed for the workflow's relative paths, the workflow entry in the doc-only allowlist, and the Actions variables; `/en-setup` now installs `bin/ensemble-lint` alone, records the cadence, and prints the machine-side commands. Loop guard 1 is a lock on the machine rather than an Actions concurrency group; guard 3's depth variable is set by the runner. **Guards:** `tests/en-sweep/runner.test.sh` drives the real runner with stubbed `codex` and `gh` against a real clone (model and effort reach argv, eligible PRs merge and ineligible do not, no result file means no merge and exit 1, a failed check leaves the PR open, the gate skips, `--force`, `--dry-run`, `--no-merge`, `--merge-only`, the lock, a dirty tree, the repo list); `tests/en-sweep/schedule-install.test.sh` renders and lints the plist without loading it. **What this costs:** the sweep now depends on one machine being on and logged in, where Actions ran regardless; its credentials are a real account's rather than a job token's, so scoping that account is the operator's job and the security model's first section; and the machine's local time replaces UTC for the cadence.
- **D102. `/en-review` is receipt-aware: it asks the receipt before its post-review check, skips only when nothing was applied, and records `targeted_tests` for `/en-ship` (amends D58, D82).** D58 built the receipt for the en-ship-to-hook handoff and D82 made `/en-ship` write one; `/en-review` sat outside the scheme. After an applied fix it ran "unit tests + lint" blind, and with nothing applied it ran nothing, so a review followed by a ship verified the same tree twice minutes apart, and a review's own run was evidence nobody could read. **Now.** After step 12, in `interactive` and `headless`, the skill asks its own copy of `ensemble-verification-receipt` for `lint,typecheck,full_suite` and, on `check-not-recorded` alone, for `lint,typecheck,targeted_tests`. Exit 0 skips lint, typecheck and tests, which an applied edit makes impossible because it moves the fingerprint. Otherwise it runs lint, typecheck and the project's `test_changed_command`, the graph-selected set (`selection: graph`; the unit suite, `selection: approximate`, when none is declared), reverts the applied edits on a regression, and on success writes `lint`, `typecheck` and `targeted_tests`, never `full_suite`. `/en-ship` asks for `full_suite` first and then for `lint,typecheck,targeted_tests`, exactly the set its own step 5 runs, so a review's receipt is honoured. `/en-build`'s receipt now also records one check per named suite in `AGENTS.md` (`unit`, `db`, `e2e`), so a later layer can ask for exactly the suite it needs; per-suite names are not partial credit, and a caller asking for `db` gets `db` or `check-not-recorded`. **Two limits chosen on purpose.** With nothing applied the check runs only on the bare branch diff, whose base is the one `/en-ship` asks about; and the receipt is written only there, because `targeted_tests` names a set selected against a base, and a receipt from `--base HEAD` would describe the uncommitted delta alone while `/en-ship` read it as its own. `report-only` runs and writes nothing (D38). The rule lives in `references/post-review-check.md`; SKILL.md carries one paragraph, since the file is pinned by the size budget, and two Phase 1 items were tightened to make room. **Guards:** `tests/lint/en-review-receipt.test.sh` pins the script copy, the verify-before-run order, the second question in both skills, the skip clause, the graph-selected set, the write line and the absence of `full_suite`, each clause negative-controlled. **What this costs:** a bare `/en-review` with no receipt now runs lint, typecheck and the targeted set where it ran nothing before, a cost `/en-ship` gives back on the identical tree; and a review that fixes something under `/en-build` runs the targeted set the full suite is about to cover anyway.
- **D103. Per-host agent model binding, `peer_*` keys, and peer resolution in every peer-invoking skill (amends D86, D100; replaces `agent-dispatch.md`'s "select nothing on Codex" rule).** One `/en-plan` run on 2026-09-06 cost about $182 because seven research agents had lost their `description:` (EN14) and every dispatch fell through to the session model; PR #85 restored the field, made the fallback carry the file's tier, and made a config file that does not parse say so. What remained structural is decided here. **Tier from the alias, no new field.** Every bundled agent declares its tier through `model:` (`haiku` retrieval, `sonnet` evidence, `opus` ceiling); a resolver, `ensemble-agent-model`, maps the alias to the tier and consults flat operator keys, per-agent override for the host first, then per-tier for the host, then the agent's own frontmatter, then inherit. No `tier:` frontmatter field is added, because the host loader's handling of unknown keys is unverified and an unregistered agent is the failure just repaired. **The asymmetry, stated.** Claude Code takes a per-call `model`, so the resolved alias is passed on every dispatch, named path and fallback alike, which puts the operator's choice on the first rung of the host's own resolution order. Codex binds a model per agent file and reads only TOML under `~/.codex/agents/`, so `setup` renders one TOML per agent with model and effort from the resolver, bound from `~/.ensemble/config.json` alone (never the checkout `setup` ran from), and a config change takes effect on the next `./setup`. The markdown files `setup` used to symlink there were never read. A repo that wants a different Codex model for one agent uses Codex's own project scope, `.codex/agents/<name>.toml`, which is documented, not generated. **Keys are flat**, `agent_model_<host>_<tier>`, `agent_effort_codex_<tier>`, `agent_model_override_<agent>_<host>`, `agent_effort_override_<agent>_codex`, because the config reader is flat by design and every existing key is; Claude values are aliases validated against the set the Agent tool accepts, Codex values are operator IDs, and Ensemble's skills, agents, templates and this document still name no ID; recorded evidence in designs, plans and the tracker names the model it observed, because a reproduction with the model redacted proves nothing. **`peer_*` replaces `review_peer_*`** (`peer_model_alias`, `peer_codex_model`, `peer_effort_override`), read with the old names as fallbacks for one release, because `/en-plan` and `/en-foundation` now resolve the peer's model and effort through `ensemble-peer-flags` too; only `/en-review` runs the diff-size ladder, and a document review passes `--effort inherit` when no override is set. `review_host_model_alias` keeps its name; only review personas consume it. **Cost:** the resolver and the config reader are copied byte-identical into every skill that dispatches an agent (EN12), guarded by a parity test that discovers carriers rather than listing them. **Not decided here:** whether a Codex session can spawn a rendered custom agent by name from a skill (EN16 U6 reproduces it), and whether the Deep plan loop cap moves (EN16 U12 records the rule and waits for two calibrated samples).
- **D104. One symmetric key scheme for models and effort, per host and per tier; the review-only host key retired; Claude effort bound at install time (amends D100, D103).** D103 left four peer keys named in two eras (`peer_model_alias`, `peer_codex_model`, `peer_effort_override`, `review_host_model_alias`) beside the agent keys' `agent_model_<host>_<tier>` shape, and an operator reading the file could not tell which side a key belonged to. Every key now reads `<thing>_<host>[_<tier>]`: `peer_model_claude`, `peer_effort_claude`, `peer_model_codex`, `peer_effort_codex`; `agent_model_<host>_<tier>` and `agent_effort_<host>_<tier>` for `retrieval`, `evidence`, `ceiling`; `agent_model_override_<agent>_<host>` and `agent_effort_override_<agent>_<host>`. Claude models are aliases, Codex models are the operator's IDs, efforts are `low`, `medium`, `high`, `xhigh` on both hosts. **Effort per peer host, not one value:** effort names do not mean the same reasoning across vendors or generations (D95), so each peer host gets its own; only the pair for the peer's host is live on a run. **`review_host_model_alias` is retired.** The review personas are the `dimension-reviewer` agent, the ceiling tier, so `agent_model_claude_ceiling` answers it through the same resolver every agent uses; one fewer key and one fewer concept. **Claude effort binds at install time.** The Agent tool has no effort parameter and Claude Code reads a subagent's effort only from the installed agent file's `effort:`; a file without one inherits the session level. `setup` therefore renders `~/.claude/agents/<name>.md` as a copy with `effort:` injected when an `agent_effort_claude_*` key or override applies to that agent, and links the checkout's file as before when none does, so with nothing set the checkout stays the source of truth. Same binding point and the same re-run-setup caveat as the Codex TOML (D103); unlike the Codex model field, this one binds today, because it is the frontmatter the host already honours. It is also the only way to run retrieval and evidence agents below the session's effort without lowering the session. **Migration:** the reader's `--legacy` is repeatable, so both retired generations map to the new names in one hop; `setup` names every retired key it finds after the merge; all fallbacks are dropped together next release (TD12).
- **D105. The unit loop verifies through one helper, commits on its exit code, and never picks its own test set (amends D53).** Two measured builds failed the same way from opposite sides. The EN16 build ran the loop as 425 separate Bash calls across six units, left 1.3M characters of their output in the window until compaction, and committed three units carrying a red assertion, because verifying and committing were separate acts of attention. The build before it picked phase-boundary suites by name and missed two tests that anchor on file *content*, which stayed red for two hours and arrived as P1s in review. **One call verifies a unit:** `ensemble-unit-verify` runs lint, typecheck and the unit's tests, writes every byte to a log under `.git/ensemble/runs/`, prints one line per check plus the tail of whatever failed, and exits `0` green, `1` failed, `3` selection empty, `4` nothing declared. Step 9e's commit is gated on that exit code in the same command, so a red assertion cannot reach a commit by inattention, and an empty selection is a finding rather than a green light. **The selection is resolved, not judged:** `ensemble-test-select` returns a tier and the reason for it, first match wins, and both `/en-build` and `/en-ship` call the same copy instead of restating the rules in two skills. The tiers are `graph` (the project's `test_changed_command`), `impact-map` (its `test_impact:` prefixes), `sibling` (the filename heuristic), `full-suite` (past sixty percent of the suite), `empty` and `none-declared`. **The cheap-suite tier is the new one, and it is opt-in:** with `--prefer-full-when-cheap`, a project whose `test_full_seconds` is at or under five minutes runs its whole suite at a phase boundary. D53's argument for targeted boundaries was made against a suite that took 2h48m across twelve launches; against a 226s suite the trade inverts, and only the whole suite finds a test that greps a file the change did not touch. **Three optional AGENTS.md keys carry it,** `test_full_seconds`, `lint_changed_command` and `test_paths_command`, in that file rather than `.ensemble/config.local.yaml` because they must reach CI and the team, and the repo-local config is gitignored. `{files}` in a command is replaced with the changed paths and `{tests}` with the selected test paths, and a command whose list is empty is skipped rather than run against nothing. Both placeholders came from this repo: the doc lint went from 68s a run to 3s once it took changed files, and `./tests/run.sh` takes `-k <pattern>` rather than a list, so the default `<test command> <paths>` form handed it a flag it rejects. **Guards:** `tests/lint/ensemble-unit-verify.test.sh`, 35 assertions over tier order, the empty case, the gate's exit codes and the substitution, with three negative controls. **Cost:** a project that declares none of these keys gets the sibling heuristic and an honest `empty` where it used to get silence, which is more work for the operator and less false confidence.
- **D106. The run ledger covers `/en-build`, because two cost analyses had to be reconstructed by hand (amends D98, extends EN16 U11).** `ensemble-run-metrics` recorded `/en-plan` runs only. The two analyses that produced this pass were assembled from commit timestamps, peer job directories and transcript archaeology, and both reported the persona timings as `Unknown`, which is the one number that would have said whether the review or the unit loop is the thing to fix. The helper now takes `unit`, `phase`, `suite` and `review` events beside `dispatch`, `peer`, `lint` and `findings`; a unit or phase is a `start` and an `end` event, so a duration is a subtraction rather than a guess, and a gate's wait time is visible as the gap between them. `/en-build` starts the ledger at step 4c and ends the report with the same `metrics: <path> (<summary>)` line `/en-plan` already writes. **What is recorded is what a run can see.** A persona's duration is whatever the host reported, and `null` when it reported nothing: an unmeasured number is recorded as unmeasured, never estimated. Compaction count and dollar cost stay unobservable from inside a run and stay unrecorded. **Cost:** the file is per clone and never committed, so evidence dies with the checkout; that is the same trade the verification receipt makes, and the alternative is a committed artefact nobody would trust to be current.
- **D107. Three context cuts: the review reads the diff from a file, lints only the changed docs paths, reuses the plan's learnings citations, and `/en-build`'s post-build protocol moves to a reference.** A build long enough to compact re-injects every invoked skill in full on every compaction. The measured session compacted 23 times, about once every 13 minutes, and paid roughly 167KB of skill bodies each time. Four cuts, each on evidence: **the diff is a file.** `/en-review` step 5 wrote `git diff <base>...HEAD` into the window, about 65k tokens for a 250KB branch diff, re-read on every later turn, although the personas and the peer each read the file themselves and the detection scans are greps. It now writes `diff.patch`, reads `--stat`, and opens hunks as findings need them. **The pre-flight lint is scoped to the changed paths.** `--scope docs/` walks the tree at 68s a run and ran five times in one build; `--scope <file>` is 3s, and the scope argument always accepted a file. **The plan's learnings citations are reused.** `/en-plan` already ran `learnings-research` and the plan carries the result, so a review whose branch has a plan reads those files and records `learnings: from-plan (<n> cited)` instead of dispatching an agent to rediscover them. **The post-build protocol is a reference.** The receipt paragraphs, both trailer schemas, the audit's report shape and the learning checkpoint's seven steps matter once, at the end of a build, and were carried from the first unit onwards; they move to `en-build/references/post-build-protocol.md`, 6.4KB out of a 49KB body, with no rule changed. The body ends the pass at 46KB rather than 43KB because D105's mechanism was added in the same pass, and the size pins were re-cut to the new numbers so the weight cannot quietly return. **What deliberately did not move:** the universal safety gates, the autonomy contract and the mandatory summary lines. Those are compliance surfaces, and a model under context pressure that skips one reference read would then omit a gate line rather than a format detail. Its guards moved with it: the assertions that anchored on those clauses now read the pair, so the property is guarded in whichever file states it. **Cost:** step 10 now costs one file read it did not before, and a reviewer reading only `SKILL.md` sees the gates but not their formats, which is the trade every reference in this repo makes.
- **D108. Phasing is removed from `/en-build`; the plan's own order stands, gates are per unit, and the checkpoint attaches to the risk (amends D25, D33, D53; retires the phase invariant).** `/en-build` re-sorted a plan's units into four risk phases, group-confirmed a whole phase with one typed string, checkpointed at each boundary, and constrained plans with a dependency-vs-phase invariant. The repo's own history says it did not earn that. **229 commits carry a phase trailer** (P1 50, P2 149, P3 30) and **none is P4**: the destructive phase, which most of the machinery exists for, has never run, though six shipped units declare `risk: destructive`. The invariant blocked a good plan once and missed a real one: EN14 linted clean through two peer iterations and was then refused at preflight, with the workaround still visible in the shipped plan (TD8), while EN01 shipped violating it and built fine (TD10). **What replaces it.** Units build in the order the plan lists them; the plan's order is the plan's decision. Every safety gate is per unit and nothing group-confirms it, which is a strengthening rather than a trade: one typed string covering three irreversible units was weaker than three typed strings, and 8b said in as many words that a P4 group confirmation skipped the per-unit destructive gate. The one property phasing did provide, reversible work proven before irreversible work, is kept two ways: `unit.destructive-order` refuses a plan that lists a destructive unit before a non-destructive one, at plan time where the fix is cheap, and step 9f runs the wider verification immediately before the first destructive or gated unit and once after the last unit commits. **Retired with it:** the six phasing triggers, phase classification, `phase-invariant.dependency-vs-risk` and its `unit_phase` helper, the `phase: P<N>` trailer, `--no-phasing`, `--from-phase`, `build.pause_between_phases`, and the category-promotion golden fixture whose whole subject was phase placement. **Backward compatible:** no plan ever authored a phase, they were derived at build time, so no existing plan changes meaning; the 229 historical trailers stay in git as a record of how those builds ran. **Cost:** on a long plan whose author put a destructive unit in the middle, nothing reorders it out of harm's way any more. The lint catches that at plan time, and a plan built with `--no-peer` and never linted could still slip through; that risk is smaller than the group confirmation it replaces.
- **D109. One definition of a good test, shared by the skill that writes tests and the skill that judges them.** Tests were written in `/en-build` and judged in `/en-review` against a rubric `/en-build` never saw: the reviewer's testing dimension had nine categories, the writer had one line about mocks. On the EN16 build the testing persona was the slowest reviewer at 627s and its findings were fixes for tests written hours earlier by a step with no rubric in front of it. `references/good-tests.md` is now carried byte-identical by both skills: what a good test is (behaviour through the public interface, reads like a specification, survives a refactor), and five anti-patterns each carrying **the tell that catches it**, because a prohibition nobody can detect is decorative. Three are adapted from Matt Pocock's `tdd` skill (MIT, credited in the file): implementation-coupled, tautological, and horizontal slicing, the last of which Ensemble had nowhere and which `Execution note: test-first` could be read as licensing. Two are Ensemble's own: reading source instead of running it, with its generated-artifact carve-outs, and mocking past the system boundary. **Seams stay `/en-plan`'s**: this file names the rule and does not restate it, because a forked seam rule is worse than a missing one. **The peer brief repeats the anti-patterns on purpose.** The peer runs as a subprocess and receives the brief's text as its prompt, so a pointer to a file it cannot follow would silently drop the dimension; `tests/lint/good-tests-reference.test.sh` fails when either side names an anti-pattern the other does not, which makes the duplication checked rather than merely tolerated. **Cost:** two files state the same five names, and a sixth anti-pattern has to be added in both places or the guard goes red, which is the point.

- **D110. `/en-plan`'s SKILL.md states each rule once; the rest moves to references it cites.** At 40,191 bytes it was the largest SKILL.md in the repo, 5x the `ce-plan` kernel it benchmarks against, and the bulk was not prose that had to exist. The finalize loop was written three times (the peer step, a `## Cross-review and finalize loop` section, and `references/outside-voice.md`), the per-unit field list restated `references/templates/plan-template.md` which step 15 loads on every run anyway, and roughly a fifth of the file argued for its own instructions instead of giving them. Four blocks that fire on a minority of runs moved to references the flow cites at the step that needs them: `plan-intake.md` (the bounded foundation read, the design-doc consumption rule and the context-sufficiency check, all skipped for a bug fix, a TD item, `--resume`, `--from-legacy` or any design-backed request), `wide-refactors.md` (expand/migrate/contract), `plan-prewrite.md` (the five quality checks and the warranted-file gate) and `plan-failures.md`. **Result: 24,353 bytes, under the 24,576 budget, so its BASELINE row is gone and the budget now checks it.** **What stayed in the flow on purpose:** the full seven-condition warranted-file gate, the peer-reject override's routing to the status-flip and auto-commit steps, the six paths that reach `open`, and the seam rules, each because a guard proved the property was genuinely lost when it moved, not merely relocated. **Cost:** a Standard run now opens two or three references it used to have inline, and every extraction carries a clause-0 assertion pinning its pointer to the step that needs it, scoped to the step so a mention elsewhere cannot satisfy it.

- **D111. `/en-review` keeps the boundaries in its flow and the procedures in references, and the envelope exists once.** At 30,715 bytes it was the second-largest SKILL.md in the repo, over four times the `ce-code-review` kernel it benchmarks against, and its densest guard surface: about seventy prose assertions across 1,314 lines of tests. **The defect found on the way in was not verbosity.** The envelope shape existed in three places: SKILL.md's complete copy, a `persona-dispatch.md` copy predating EN08 and EN11 (no `lite_gate`, `verification_pass`, `applied_fixes`, `peer_decision` or `diff_base`), and a `finding-schema.md` sentence pointing readers at the stale one. `references/review-report.md` is now the single owner of what a run returns: the envelope, the four mandatory outcome lines, and the markdown summary, which move together because four assertions cross-check the JSON example against the summary example. **The outcome-line rule is stated once.** `lite_gate:`, `peer_decision:`, `review_fixes:` and `verification_pass:` each carried their own copy of "every run emits exactly one" and "the line is derived from its structured object", and the four wordings had already drifted; each step now keeps the obligation and the reference owns the forms. **`references/mutation-protocol.md` holds the two-phase procedure**, while SKILL.md keeps the four rules that bind every mode, because a reader who never opens the reference still must not cross them: authorization precedes mutation, `manual` is never applied without an explicit pick, a P0 halts all automatic mutation, and the recorded `applied_fixes[]` bounds the tree delta. **Result: 22,385 bytes, under the 24,576 budget, so its BASELINE row is gone.** **Cost:** a `--cross` run opens two references it used to have inline, and the guards that moved now assert against the reference plus a clause pinning the pointer in the step that needs it, scoped per step or per line so a mention elsewhere cannot satisfy them.

- **D112. A verification receipt accumulates across layers on an unchanged tree, and no layer records a check it skipped.** `write` replaced the whole document, so the second writer erased the first's evidence. The observed sequence: `/en-build` proved `full_suite`, `lint`, `typecheck` and each named suite; `/en-ship` ran seconds later on the identical tree, skipped its checks against that receipt, and then **wrote its own receipt claiming `targeted_tests`**, dropping `full_suite`. The project's pre-push hook then asked for `full_suite`, got `check-not-recorded`, and re-ran the whole suite the build had just paid for, 38 seconds of database setup and 96 tests on a tree three layers had already verified. **Two changes, and both are needed.** A write now MERGES into an existing receipt when the fingerprint and repo match, keeping the union of checks, every writer's name and the **oldest** `written_at`, because refreshing the clock would let a one-second lint check renew an hour-old suite result indefinitely. And when `/en-ship` skips on a valid receipt it now writes nothing at all, because recording `targeted_tests=passed` for tests that never ran is a false claim the next layer skips on, which is worse than no receipt. **The fence is the fingerprint**, unchanged: a differing tree replaces the receipt outright, since the prior checks describe code that is no longer there. That half is what makes merging safe and is the part to preserve in any future change; `tests/verification-receipt/receipt.test.sh` fails when either the merge or the fence is removed. **Not changed: CI still never reads a receipt.** It is the independent authority on a tree no local layer can vouch for, and a receipt is a local optimisation worthless to anyone but the checkout that wrote it.

- **D113. `/en-ship` bundles the steps that were failing, and the skill shrank by gaining scripts.** A field run spent 47 minutes, 40 of them in a watch loop that produced nothing. Step 13 described the loop in prose, so the agent hand-wrote a poll script; that script swallowed `gh`'s stderr and looped in silence against a sandbox blocking GitHub with a TLS certificate error, and the PR merged with a review finding unaddressed. Three prose-to-script conversions followed, each replacing a step the skill was asking every run to re-derive. **`ensemble-ship-watch`** polls and reports a state, under two rules: a heartbeat fires on wall-clock time whether or not anything changed, and **two consecutive `gh` failures end the watch with a classified reason** rather than reading as "CI still running". `checks-settled` means every check has a conclusion, not that they passed, so a red check exits 0 and the caller repairs it: "not clean" and "cannot look" sharing an exit code is how the original defect hid. The script never fixes, pushes or merges; the trusted-source gate and the repair-cycle cap stay in the skill where they are guarded. **`ensemble-secret-scan`** implements the pattern table that was previously grepped by hand, which mattered twice over: `grep -E` has no portable inline case-insensitive flag, so five of the fifteen patterns were silently case-sensitive, and `grep -n` prints the matching line, which moves the credential into the terminal and the PR body. It masks every preview after four characters and scans added lines only. **`ensemble-pr-body`** derives Summary, Verified locally and the plan link from artifacts that already exist, and prints "No test run recorded for this branch" when there is no result rather than letting a plausible checklist stand in for one. **The size result is the part worth generalising: 27,407 to 24,565 bytes while adding three helpers**, because a bundled script displaces more prose than its pointer costs. Its BASELINE row is gone. **A defect found by writing the fix:** the first draft of the watch script called its emit-and-exit from inside a command substitution, so the exit killed only the subshell and the loop spun forever, reproducing the exact hang it was written to prevent; the wrapper now returns a status and only top-level code emits, and that trap is recorded in the file's own header.

- **D114. The verbosity pass `/en-ship` never got, and the guard pattern it exposed.** D113 took en-ship from 27,407 to 24,565 bytes, but that was a side effect of three scripts absorbing prose plus byte-shaving to clear a pin, not the structured review en-build, en-plan and en-review each received. The measurement that made it obvious: en-ship was the largest of the four at 24,565, the longest at 229 lines against 147 to 194, and carried **7 references against their 12 to 19**. That last figure is the signature of content that should have moved out and did not. The benchmark was also never taken: CE splits this work across `ce-commit-push-pr` (7,854) and `ce-babysit-pr` (7,877), 15,731 for both against en-ship's one file. **Result: 20,655 bytes over 178 lines**, the smallest of the four. What moved: a `## Bundled scripts` list that was both redundant and already stale (four scripts listed, seven carried), a `## Reference files` list of the kind deleted from three skills before it, the run report and the watch loop's five exit states into `references/ship-reporting.md`, and the delegate contract's permitted set into `references/watch-loop.md`. **What stayed on purpose:** the delegate's EXCLUDED actions, in the flow and in the `**Excluded:**` layout, because a reader who never opens the reference must not let a delegate merge, and because `tests/lint/en-resolve-pr-seam.test.sh` keys on that same layout to check both sides of the contract list the same exclusions. Rewriting it as prose turned that guard red, which is the drift check working. **The pattern worth generalising is a self-contradictory guard.** en-ship-preflight asserted `"the selection comes from the shared helper, not from re-derived prose"` and then, in the next three lines, required the skill to name `test_changed_command:`, `test_impact:` and the sibling heuristic, which is exactly the re-derived prose the first assertion forbids. Roughly 700 bytes existed because a test demanded them. The tier order was already proven behaviourally in the same file, driving the real helper on a throwaway repo, so the prose assertions were checking a copy of something already tested; they now read the helper. **When a skill restates a script's header, check whether a guard is what is holding it there.**

- **D115. A Codex session dispatches a bundled agent by name, and TD13 closes on upstream evidence rather than a workaround.** EN16 rendered `~/.codex/agents/<name>.toml` for every bundled agent with the resolver's model and effort, then found that a Codex session could not select one: `spawn_agent` took `task_name`, `fork_turns` and `message` only, a child spawned for a research agent carried `agent_role: null`, and it ran on the parent's model. So the per-host binding EN16 built reached Claude Code alone, and TD13 recorded the gap with the condition to re-test: *when Codex's spawn tool gains an agent or role selector*. It has. On Codex CLI 0.153.4 the tool takes `agent_type`, `model` and `reasoning_effort` as well, and `agent_type` loads the named TOML as a high-precedence config layer on the child. **Reproduced 2026-09-10 in two read-only `codex exec` runs**: the session listed all six parameters, and a child spawned with `agent_type` recorded `agent_role: <name>` and the TOML's model and effort against the parent's own. `./setup` needed no change; it already wrote the fields `agent_type` resolves against. **The reference now says how to dispatch, and what to do when it cannot**: a CLI without the parameter takes the agent's body inline and reports that the child ran on the session model, so the degraded path is visible rather than silent. **The lesson worth more than the fix:** asked to name its own model, the child answered with one that was neither the parent's nor the TOML's. A model's self-report is not evidence about its own deployment, and the first version of this reproduction would have concluded the opposite from it. The rollout is the evidence, and `agent-dispatch.md` now says so beside the instruction. **Cost:** the dispatch instruction is version-sensitive, which is why the fallback is documented rather than assumed away.

- **D116. `/en-setup`'s install is a script, and the BASELINE list is empty.** en-setup was the last skill over the 24,576-byte budget and the only one never reviewed. Its shape was unlike the other four: **315 lines against their 147 to 194, and 2 scripts against their 6 to 10**, with no section over 2,736 bytes. Nothing was bloated; the length was 21 numbered steps, and six of them were about 2 KB telling a model to run twenty commands whose arguments never vary. The tell was already in the file: the final verification phase existed to check that the sequence had happened, and said why, "long mechanical sequences drop steps under context pressure". A step whose job is catching the previous six failing is a script's job description. **`ensemble-scaffold` does all six** and reports `created` or `exists` per artifact, exit 1 naming what it could not create. It is **idempotent by construction** rather than by six separate prose promises to be idempotent. Three properties it will not trade: an existing seed file is never overwritten, `.gitignore` is appended and re-read rather than rewritten, and `bin/ensemble-lint` is the one thing refreshed against drift, because it is a copy and `exists` must mean byte-identical. The verification phase still walks every artifact independently from its own list, because a scaffold that signs off on its own writes is one report where two were the point. **Also: `references/setup-optional-installs.md`** for the fixed blocks a run prints rather than decides (the sweep machine's commands, the receipt notice, the next-steps list), and the fifth and last `## Reference files` deleted, with the four files named nowhere else moved to the steps that read them. **Result: 30,301 to 24,524 bytes over 16 steps, and the BASELINE list is now empty**, so the next skill over budget is a regression rather than a legacy. **Cost:** State 2 renumbered from 21 steps to 16, and D63 and D92 both record these citations drifting before, so they were rewritten from an explicit map and checked against `intra-file-step-citations` plus four pinned `cites()`. **A test bug worth the note:** the scaffold suite's fixture helper incremented its counter inside `$(fresh)`, which runs in a subshell, so every case got the same directory and one test's fixture data was still present when a later test asserted against it. `mktemp -d` needs no shared state.

- **D117. The helper that does the work records it, and the ledger is addressed through a file rather than an environment variable.** `ensemble-run-metrics` had existed since D106 and **had never written a ledger in this repo**: recording was the model's job at documented call points, and the model forgot, so the peer-in-the-fix-loop question sat parked for weeks on data that was never accumulating. EN17 moves recording into the four helpers that already hold the answers, and the whole design follows from one constraint. **The env var the design specified cannot work here.** Every tool call in the host is a fresh shell, so `ENSEMBLE_RUN_LEDGER` exported by the call that ran `start` is gone by the call that runs `ensemble-unit-verify`; keeping it would have put `ENSEMBLE_RUN_LEDGER=… bash …` on every helper invocation, which is exactly the bookkeeping-the-model-forgets the design rejected the status quo for. So the ledger is addressed through `runs/active`, a per-repo file that survives across shells and dies with the clone, and the variable is demoted to an override for tests. **That file is append-only**: `start` appends `+`, `finish` appends `-`, and resolution replays the tail backwards for the newest unclosed run. It began as a mutable stack and three peer passes found defects in it that were all properties of rewriting a shared file, which `docs/learnings/repeated-review-rejects-are-a-design-signal-2026-09-11.md` records. **Every settled decision that rode on it holds unchanged**, and this version needs no skill changes at all for addressing. **Liveness is the absence of a `finish` event anywhere in a ledger**, not a last-line test, so a late event cannot resurrect a closed run. **A per-kind key allowlist decides what reaches disk**, in one place, dropping unknown keys loudly; `/en-review`'s corroboration split is the single model-emitted exception, because `ensemble-peer-invoke` has no notion of corroboration and no helper can observe it. **The rollup is published before the ledger closes** and is idempotent on skill plus `run_id` across both retained generations: the other order loses a run's only durable record whenever the store is briefly unwritable. Its check, rotation and append are one transaction under a lock on the rollup file, because two unsynchronized renames destroy a 5 MB generation and two unsynchronized checks both append the same run. **Cost, and the lesson:** the isolation guard PR #108 added had four holes this work fell through in one session — it discovered writers by a redirection operator, matched its own explanatory comments, checked the file rather than each driving line, and accepted an export placed below the first call. The suite wrote 156 lines into the operator's store while the guard stayed green. **Three more guards in the same build had the same shape**, each shown decorative only by mutation: a per-skill assertion satisfied for all five carriers by one shared reference, a suite that drove an emitter eighty-four lines above its own override, and a whole-file exemption marker a file could carry while genuinely executing a writer. A guard that passes is not evidence until a negative control has made it fail for the reason you think it checks.
---

## 5. Skill Catalog

Sixteen skills total: the lifecycle skills (brainstorm → foundation → plan → build → review → qa → learn → ship → resolve-pr) plus orthogonal skills (`en-debug`, `en-flow`, `en-guardrail`, `en-loop`, `en-setup`, `en-simplify`, `en-sweep`). All prefixed `en-` to namespace cleanly alongside other plugins.

The count read **fourteen** until 2026-09-03, and had done since `en-flow` and `en-simplify` were added: both have §5.1 rows, neither was in the sentence. A lint asserted the literal word "Fourteen", so the check that existed to notice a skill appearing was the reason the number could not be corrected. It now counts `skills/` and compares.
...
| 11 | `en-sweep` | Event-driven doc-drift cleanup on every PR merge to `main`. Opens auto-merging doc-only PRs. **Continuous monitoring** (opt-in): dead-code + dep-vuln scans with size-based triage (trivial → TD; pattern/severe → draft plan in `docs/plans/active/`). | Repo state (post-merge) | Auto-merging cleanup PRs + TD entries + draft plans | Off | Yes |
| 12 | `en-guardrail` | Always-on `PreToolUse` hook that prompts before destructive Bash commands (recursive rm, DROP TABLE, force-push, terraform destroy, aws s3 rm --recursive, etc.). Localhost+test/dev DB exemption. Per-command bypass via `ENSEMBLE_GUARDRAIL=off`. Installed globally via `~/.claude/settings.json` or project-scoped via `<repo>/.claude/settings.json`. | Bash tool input (intercepted) | Permission prompt or pass-through | Off | Optional |
| 13 | `en-setup` | Project-level bootstrap and diagnostics. Detects state (1 / 2 / 3); for State 2 retrofit: archives non-conforming legacy plans, creates skeleton, generates `AGENTS.md` + `CLAUDE.md`, installs `.github/workflows/en-sweep.yml`, offers guardrail / Anthropic Code Review action / Codex Code Review action installs, checks repo-level `allow_auto_merge`, surfaces `CONTEXT.md seeding` offer. | Repo state | Project skeleton, config files, GH Action workflows, diagnostic report | Off | Yes |
| 14 | `en-loop` | Bounded, objective-driven autonomous loop (wraps the `gnhf` CLI): one committed test-gated slice per iteration until an evidence-based stop condition; branch-level cross-agent review at checkpoints (`--review-every N` + loop end). Manual-invoke only; never auto-merges. | Objective + evidence-based stop condition | Reviewed feature branch + gnhf exit summary | On (branch-level, at checkpoints) | Yes |
| 15 | `en-flow` | Pipeline runner: chains skills end to end for a single unit of work. | A task description | A completed pass through the chained skills | Off | Optional |
| 16 | `en-simplify` | Behaviour-preserving simplification of the branch diff across reuse / quality / efficiency. Called by en-build post-build and usable ad hoc. | Branch diff vs base | Simplified working tree, unchanged behaviour | Off | Optional |
...
- **Process (high-level).** (`skills/en-foundation/SKILL.md` is canonical.)
  ...
  3. Right-size depth; run the discovery loop from `references/foundation-questions.md` in frontier rounds, one round per group with a recommended answer per question (one per turn on Lightweight and for rigor probes), asking only what the ledger left open and confirming detected groups in one line; the ledger lives on disk under `/tmp/ensemble/en-foundation/<run-id>/` and the draft is written from it (D97).
...
- **Process (high-level).** (`skills/en-build/SKILL.md` is canonical; this is a summary.)
  ...
  4. Units build in the plan's own order, with irreversible work last (`unit.destructive-order`, D108). Universal safety gates, stated once in 8b: `risk: destructive` needs the typed `"run unit U<N>"`, `gated: true` needs y/skip/abort every time, no flag disables either.
...
- **Flags.** `--review cross|peer|none`, `--no-simplify`, `--unit`, `--from`, `--dry-run`, `--finalize-only`, `--commit-wip`, `--re-baseline`. Standing policy (`worktree`, `strict_destructive`, `pause_between_phases`, `learning_checkpoint`) lives in `.ensemble/config.local.yaml`.
...
- **Process (high-level).** (`skills/en-review/SKILL.md` is canonical; this is a summary.)
  ...
  4. Finalize the effort tier against the ladder; requirements coverage (the spec axis) against the plan's units, reported as its own findings. Reviewer models are operator settings read through `ensemble-config-get`: `peer_model_<host>` and `peer_effort_<host>` for the peer (retired spellings read for one release, D103, D104); the personas resolve as the ceiling-tier agent (`agent_model_claude_ceiling`), and their effort is the installed file's `effort:`, rendered by `setup` from `agent_effort_claude_ceiling` or overridden by a project-level agent copy (D100, D104).
  ...
  6. Synthesize (two-source reconciliation under `--cross`); confidence gate (default 7; P0 always surfaces); two-phase mutation protocol with a frozen authorized set, `applied_fixes[]` derived from the tree delta, and the mandatory `review_fixes:` line (D42); one severity-gated verification pass when a P0/P1 was addressed, with the mandatory `verification_pass:` line (D80); the post-review check asks the verification receipt first, skips when the tree is already proved and nothing was applied, and otherwise runs lint, typecheck and the graph-selected set, recording `targeted_tests` for `/en-ship` (D102); markdown summary plus JSON envelope, written to `/tmp/ensemble/en-review/<run-id>/`.
- **Reference files.** Every run: `references/host-detect.md`, `references/peer-brief.md`, `references/peer-contract.md`, `references/severity.md`, `references/finding-schema.md`, `references/outside-voice.md`, `references/peer-model-policy.md`, `references/diff-signal-detection.md`, `references/review-confidence-gating.md`, `references/tech-debt-tracker-format.md`. Gated on `--cross`/`--host`: `references/persona-dispatch.md`, `references/research-dispatch.md`. Under `--lite`: `references/peer-brief-lite.md`. At the post-review check: `references/post-review-check.md`.
...
- **Purpose.** Scheduled doc-drift cleanup (D14, D27, D101). launchd on a dedicated Mac runs `scripts/ensemble-sweep-runner` on a cadence (`daily` / `weekly` / `monthly`, local time; default weekly, Monday 09:00), which drives each checkout listed in `~/.ensemble/sweep-repos` through **Codex** and then merges the doc-only PRs it opened once their checks are green. Also by hand as `/en-sweep`, or `ensemble-sweep-runner --force`. Strictly doc-only: code-level findings file as TD entries, never as PRs.
- **Activity gate.** `ensemble-sweep-activity-check` finds the last sweep-authored commit on the default branch by the scopes `chore(sweep|arch|plans|learnings|maps):` and counts non-sweep commits since; zero skips the repo silently. Every scope in that list must be one a human would not type; `docs` was one and silenced the gate (D65). `--force` bypasses the gate.
- **Execution.** Per repo the runner refuses a dirty tree (ignoring the skill's own `.ensemble/` files), fetches and fast-forwards the default branch, runs the gate, then `codex exec --json -s workspace-write` with network on, `approval_policy=never`, the model and effort from `sweep.model` / `sweep.effort` in that checkout's `.ensemble/config.local.yaml` (else `ENSEMBLE_SWEEP_MODEL` / `ENSEMBLE_SWEEP_EFFORT` from the plist, else Codex's default), and a prompt that invokes the skill unattended. **The proof the skill ran is `.ensemble/sweep-result.json`**: no valid file, no merge, and the repo is logged as failed. No host detection and no peer inside the skill.
- **Process (high-level).** (`skills/en-sweep/SKILL.md` is canonical.) Recursion guard; loop guards (one lock per machine and the depth cap in the runner, no-material-diff inside the skill); `bin/ensemble-lint` over `docs/`; `/en-learn --lint`; architecture drift via `repo-research`, dispatched first; plan-lifecycle and pointer-map drift; opt-in continuous monitoring triaged into TD entries or draft plans; categorize into doc batches; stage each batch and verify it with `ensemble-doc-only-check`; one PR per batch on `en-sweep/<sha>/<batch>`, capped by `sweep.max_prs_per_run`; `/en-review --mode report-only` per PR, never mutating; **record** `merge_eligible` per PR in `.ensemble/sweep-result.json` (no P0/P1 and `auto_merge_enabled` not false); write `.ensemble/sweep-summary.md`. The skill never merges: the runner waits for each eligible PR's checks and runs `gh pr merge --squash --delete-branch`; a failed check, a timeout or a `BLOCKED` merge state leaves the PR open with the reason logged.
- **Security.** The runner acts as the sweep machine's own `gh` identity, whose repo permissions are the whole scope (scope it: a dedicated account or a fine-grained token with contents and pull-requests write on the swept repos); Codex sandboxed workspace-write; branch protection respected, nothing forced or approved; doc-only allowlist enforced at runtime; fail-closed on a missing result file (`references/sweep-security-model.md`).
- **Configuration.** `.ensemble/config.local.yaml` `sweep:` block, read from the checkout on the sweep machine: `enabled`, `schedule` (informational), `model`, `effort`, `max_prs_per_run`, `auto_merge_enabled`, `continuous_monitoring.*`, the triage thresholds and `max_drafts_per_run`. Cadence, the machine-wide default model and effort, and the two timeouts live in the launchd plist written by `scripts/install-sweep-schedule`.
- **Reference files.** `references/sweep-checks.md`, `references/sweep-loop-guards.md`, `references/sweep-security-model.md`, `references/tech-debt-tracker-format.md`, `references/doc-lints.md`, `references/learn-lint.md` (shared with `/en-learn`), `references/architecture-update-rules.md`; scripts `ensemble-sweep-runner`, `install-sweep-schedule`, `ensemble-sweep-activity-check`, `ensemble-doc-only-check`, `continuous-monitor`, `triage-findings`.
...
- **One opt-in round.** Every probe runs in one message after sub-variant detection, then every opt-in the probes left open (plan archival, sweep cadence, local config, guardrail scope, gnhf, the review action, `REVIEW.md`, the lint PR check) is put to the user in a single numbered round with recommended answers; the install then runs to the final verification without stopping (D96).
- **Idempotent.** A second run on a settled repo changes nothing; declines are recorded in config rather than re-asked.
- **Reference files.** `references/setup-state-detection.md`, the `AGENTS.md` / `CLAUDE.md` templates and merge rules, the review-action and lint-check workflow templates, `references/templates/config-local-example.yaml`, `references/templates/ensemble-lint` (the lint it installs), the learnings index and log formats and `references/glossary-rules.md` for seeding, `references/templates/review-md-template.md`, `references/layout-migration.md` (read to describe, never run); scripts `check-health`, `ensemble-classify-plans`, the CI wrapper (`scripts/en-sweep-ci`), `ensemble-sweep-activity-check`, `ensemble-doc-only-check`.
...
- **Default to mid-tier model on the peer.** A Claude peer runs on the `sonnet` alias unless `peer_model_claude` says otherwise; a Codex peer inherits the operator's `~/.codex/config.toml` model unless `peer_model_codex` names one. `peer_effort_claude` and `peer_effort_codex` pin the effort tier per peer host (D104). Keys are flat and layered, repo `.ensemble/config.local.yaml` over `~/.ensemble/config.json` (D45, D100, D103); Ensemble's own files name aliases, never model IDs (D44).
...
**Optional (added when valuable):** `docs/golden-principles.md` (recommended once `en-sweep` is in regular use), `docs/quality.md` (large projects), `docs/references/` (added on first `en-learn --pack`), `docs/designs/` (added on first `en-brainstorm`).
...
- `docs/foundation.md`, `docs/architecture.md`, `docs/golden-principles.md`, `docs/quality.md`
- `docs/designs/`
...
Abridged: `setup` also writes the other ten `agent_model_<host>_<tier>` / `agent_effort_<host>_<tier>` keys as `null`; the full set with its semantics is in `.ensemble/config.local.example.yaml` (D104).
...
Model choices and CLI flag specifics (`--output-format json`, `--max-turns`, etc.) are *defaults to verify*, not promises. This document names model tier aliases only; concrete model IDs live in the operator's config (D44, D100). Both ecosystems evolve quickly. During implementation:

- Concrete model IDs live in `~/.ensemble/config.json` (or the per-repo overrides) — never in skills, agents or this document; skills and agents name tier aliases.
...
- **Q11 → A11.** ~~`docs/core-beliefs.md` is seeded from a templated starter.~~ **Retired 2026-09-10 (TD4).** No skill ever read the starter and no skill ever created the artifact, so the capability was documented, shipped and unreachable. The starter is gone; this line is kept struck through because an answer that quietly disappears reads as one nobody asked.
