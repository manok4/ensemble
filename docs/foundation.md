...
- **Process (high-level).** (`skills/en-review/SKILL.md` is canonical; this is a summary.)
  ...
  4. Finalize the effort tier against the ladder; requirements coverage (the spec axis) against the plan's units, reported as its own findings. Reviewer models are operator settings read through `ensemble-config-get`: `review_peer_model_alias` / `review_peer_codex_model` / `review_peer_effort_override` for the peer, `review_host_model_alias` passed as the Agent tool's `model` to every persona; host effort is the `dimension-reviewer` frontmatter, overridable by a project-level agent copy (D100).
