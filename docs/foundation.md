...
- **Configuration.** `.ensemble/config.local.yaml` `sweep:` block: `enabled`, `schedule`, `max_prs_per_run`, `auto_merge_enabled`, `continuous_monitoring.*`, the triage thresholds and `max_drafts_per_run`. The CI timeout is `timeout-minutes:` in the workflow, not a key. The runner's model and effort are repository Actions variables, `ENSEMBLE_SWEEP_MODEL` and `ENSEMBLE_SWEEP_EFFORT`, passed by the workflow to `scripts/en-sweep-ci`; the gitignored YAML never reaches CI (D100).
