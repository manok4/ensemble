...
- **Process (high-level).** (`skills/en-review/SKILL.md` is canonical; this is a summary.)
  ...
  4. Finalize the effort tier against the ladder; requirements coverage (the spec axis) against the plan's units, reported as its own findings. Reviewer models are operator settings read through `ensemble-config-get`: `peer_model_alias` / `peer_codex_model` / `peer_effort_override` (the `review_peer_*` spellings are read for one release, D103) for the peer, `review_host_model_alias` passed as the Agent tool's `model` to every persona; host effort is the `dimension-reviewer` frontmatter, overridable by a project-level agent copy (D100).
...
- **Default to mid-tier model on the peer.** A Claude peer runs on the `sonnet` alias unless `peer_model_alias` says otherwise; a Codex peer inherits the operator's `~/.codex/config.toml` model unless `peer_codex_model` names one. `peer_effort_override` pins the effort tier. Keys are flat and layered, repo `.ensemble/config.local.yaml` over `~/.ensemble/config.json` (D45, D100, D103); Ensemble's own files name aliases, never model IDs (D44).
...
```json
{
  ...
  "peer_model_alias": null,
  "peer_codex_model": null,
  "peer_effort_override": null,
  "review_host_model_alias": null,
  "agent_model_claude_evidence": null
}
```

Abridged: `setup` also writes the other eight `agent_model_*` / `agent_effort_codex_*` tier keys as `null`; the full set with its semantics is in `.ensemble/config.local.example.yaml`.
