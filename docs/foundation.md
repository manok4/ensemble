...
- **D26. `en-learn` auto-runs after `en-qa`, AND fires as a structured checkpoint at `en-build`'s completion.** (Relocated from en-ship's preflight to en-build's completion by EN04 — see D38.) Soft auto-invoke in en-qa is a freshest-point capture (immediately after the work, while context is hot); user can decline. The **en-build completion checkpoint** is the structured, non-droppable capture point — a structured step (not a soft prompt) that detects "X commits since last capture" and surfaces a yes/skip/details prompt at build completion. Records `learning_checkpoint: <captured (N learnings) | intentionally_skipped | up_to_date | ci_environment>` in the en-build report so missed captures are auditable rather than silently dropped. Capture happens at the point of insight, while the user is present, before en-ship (now hands-off) ships. Baseline detection uses the `| <head-sha>` field on `capture` entries in `docs/learnings/log.md` (precise) with legacy date-fallback for older entries.
...
- **D38. Hands-off `/en-ship` with CI-hosted self-heal (EN04).** `/en-ship` is **hands-off by default** (a deliberate breaking change to its prior stop-and-ask contract, acceptable pre-1.0): preflight checkpoints (scope-confirm, plan-completion) auto-resolve, and a mergeable PR is produced without a live session. **Safety floor** — secret-scan match, push to the default branch, and destructive-guardrail hits always hard-stop, even hands-off; `--interactive` restores the prior prompts. The **learning checkpoint is relocated from en-ship preflight to `/en-build` completion** as a structured, non-droppable step (updates D26's location; the anti-drop rationale from the PR #18 fix is unchanged — capture now happens at the point of insight while the user is present). The plan-completion checkpoint stays in en-ship as the lifecycle backstop but auto-`y`s a verifiably-complete build under hands-off. Session-babysitting is removed by a **CI-hosted self-heal watcher** (`en-ship-watch.yml`, installed by `/en-setup`): event-driven on a failed CI `workflow_run` (named at install time, so it never self-triggers) for PRs labeled `en-ship-watch`, it drives `/en-resolve-pr` headless in the runner to fix failures, bounded to 3 attempts (a commit trailer advances the cap), **branch-only writes on same-repo PRs only** (fork PRs escalate; never the default branch), escalating by dropping the label + commenting when it can't safely proceed. The fix agent reuses en-sweep's auth secrets, but the fix **push** needs a workflow-triggering token (`EN_SHIP_WATCH_TOKEN`, a PAT/App token) because a `GITHUB_TOKEN` push does not retrigger Actions CI; without it the watcher escalates rather than pushing a fix CI would ignore. Default stops at a green, mergeable PR; `--auto-merge` arms `gh pr merge --auto --squash`. GitHub Actions is the daemon (no local daemon shipped); en-ship degrades gracefully and never reports hands-off success when the watcher is absent.
