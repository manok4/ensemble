...
Fifteen skills total: the lifecycle skills (brainstorm → foundation → plan → build → review → qa → learn → ship → resolve-pr) plus orthogonal skills (`en-cross-review`, `en-debug`, `en-guardrail`, `en-loop`, `en-sweep`, `en-setup`). All prefixed `en-` to namespace cleanly alongside other plugins.
...
| 7 | `en-learn` | Compounding wiki maintainer. **`capture`** (default): file a learning + sync architecture / foundation / plan + maintain cross-refs. **`ingest <path-or-url>`**: read external source. **`--pack <lib>`**: flatten library docs. **`--refresh`**: audit stale entries. **`--lint`**: graph health. **`--migrate`**: seed `patterns/` from existing codebase (one-time, retrofit). | Commits/branch, URL, library, or codebase | Learning doc, doc updates, index/log updates, cross-refs, library reference, or bootstrapped patterns | Off | Optional |
...
- **Modes.** `capture` (default), `ingest <path-or-url>`, `--refresh`, `--pack <library>`, `--lint`, `--migrate` (one-time retrofit; seeds `patterns/` from existing codebase via `repo-research`; entries flagged `source: bootstrap` and `requires_validation: true`, default `confidence: 6`).
