...
peer_review_plan_hash: 4cf0bea65c6be967d21d65673631ef36747df46183f8e132e107c44e5f72fb0e
peer_review_resolutions:
  - finding_id: "1-1"
    ...
    status: applied
    rationale: U12 added; en-ship, en-review and en-foundation now open and close their own runs, guarded by a lint
    location: U12
  - finding_id: "1-2"
    iteration: 1
    severity: P1
  ...
  - finding_id: "2-4"
    ...
    status: applied
    rationale: every terminal outcome is routed through the one reporting step that holds finish, and the lint gains a bypass fixture that must fail
    location: U12
  - finding_id: "2-5"
    iteration: 2
    severity: P1
...
- **Approach:** `/en-build` and `/en-plan` already call `start` and `finish`; the three skills that gain emitters do not, and without the pair their helpers resolve no ledger and record nothing. This is the gap that would have made U4 through U7 look implemented and produce no data for any skill but `en-build`. Each of the three gets the same two lines `/en-build` carries: `start --skill <name>` before its first measured operation, and `finish` in its reporting step. "Finish on every terminal path" is unverifiable as prose, so the structure carries it instead: each skill's failure protocol is routed so that **every** terminal outcome, including the early failure exits, passes through the one reporting step that holds the `finish`, and the lint asserts no terminal branch bypasses it. That turns a promise into a property a test can read. The nesting the design describes then works without anyone writing it: `/en-build` invoking `/en-review`, and `/en-ship` invoking `/en-resolve-pr`, each get a child ledger and a parent pointer from `start` alone. The guard is a lint over the skill bodies, because these are prose steps the model executes: for every skill carrying `ensemble-run-metrics`, assert a `start --skill <that skill>` call and exactly one `finish` call site, assert both are reachable from the skill's flow, and assert that every row of the skill's failure-protocol table either routes to the reporting step by name or is a non-terminal row. A skill whose failure table names a terminal outcome that does not reach the reporting step fails, which is the bypass this finding is about.
- **Risk:** medium
- **Category:** observability
...
- **Test scenarios:**
  ...
  - *Error / failure path (bypass fixture):* a fixture skill whose failure-protocol table names a terminal outcome that exits before the reporting step → the lint fails, naming that row. The same fixture with the row routed through reporting passes. This is the assertion that makes "finish on every terminal path" real rather than aspirational.
...
- **Verification:** the new lint green on all three skills; `tests/lint/skill-size.test.sh` green with `BASELINE` still empty; `tests/parity/script-parity.test.sh` green across all `ensemble-run-metrics` copies. Negative controls: delete the `finish` line from `skills/en-ship/SKILL.md` and confirm the lint goes red naming `en-ship`; change one `start --skill` to the wrong skill name and confirm the matching assertion goes red; point one failure-protocol row at an exit that skips the reporting step and confirm the bypass assertion goes red.
...
- **Files:**
  ...
  - `tests/lint/ensemble-verification-receipt.test.sh`
