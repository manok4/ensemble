...
> - 2026-09-08 (build record, /en-build): 14 units built on plan/agent-model-control. Recorded facts the units asked for: U2 live check, a Codex peer call with the translated fragments ran on the configured model at effort low (rollout turn_context); U4, the resolver flips repo-research sonnet/opus from a repo-local key (a live dispatch carrying the configured model is still pending, spec-3); U6 outcome 3, Codex 0.153.2 cannot spawn a custom agent by name (TD11, design doc); U10 timings in this repo, single-file 5.2 s vs directory 4.8 s with one active plan; U13 not reproduced under zsh 5.9 (test ships as the guard); U14 not reproduced outside the sandbox, macOS mktemp falls back silently (lint_mktemp ships). Pending recorded runs: U9 --research (three scenarios) and U11 first metrics file, after install. U12 rule recorded, samples pending (ship scope deferred). Post-build: simplify applied 1; cross-agent review 44 records, 8 P1 all applied, verification pass approve; full suite 145/145. TD12 filed for the legacy-key removal.
